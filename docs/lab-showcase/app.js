(() => {
  ...
  function dossierBody(t){
    ...
    return `<div class="dossier-body"><div class="dossier-entry">${dossierSection("Когда пригодится",story.useCases)}<section class="dossier-section dossier-start"><h3>С чего начать</h3><p>${esc(start)}</p>${invoke?`<code>${esc(invoke)}</code><p class="dossier-caption">Навык задаёт порядок работы вашему агенту. Результат зависит от доступных ему файлов и инструментов.</p>`:''}</section></div><div class="dossier-flow">${dossierSection("Как проходит работа",story.capabilities,"dossier-steps",true)}<aside class="dossier-result">${dossierSection("Результат",story.outcomes)}${dossierSection("Куда сохраняется",t.writes)}</aside></div><div class="dossier-context">${dossierSection("Что подготовить",t.inputs)}${dossierSection("Данные и хранение",t.storage)}${dossierSection("Проверки и границы",[...arr(t.gates),...arr(t.limits)])}</div>${window.LAB_OBSIDIAN_SETUP?.render(t.id)||''}<footer class="dossier-footer">${t.adoption?.installNote?`<p>${esc(t.adoption.installNote)}</p>`:''}${install.length?`<details class="dossier-install"><summary>Команды подключения</summary><pre class="setup-install"><code>${install.map(esc).join("\n")}</code></pre></details>`:''}${src.length?`<div class="dossier-links">${src.map(x=>`<a href="${esc(x.url)}" target="_blank" rel="noopener">${esc(x.label||"Исходный код")} ↗</a>`).join('')}</div>`:''}</footer></div>`;
  }
  ...
  function openQuickTool(processId,toolId,trigger){
    ...
    dialog.innerHTML=`<article class="dossier" style="--accent:${esc(process.color||"#b9ff66")}"><button class="quick-close dossier-close" type="button" aria-label="Закрыть карточку">×</button>${dossierHead(tool,process,"quick-tool-title")}${dossierBody(tool)}</article>`;
  ...
  function bindSkillCatalog(root,p){
    ...
    root.querySelectorAll('[data-skill]').forEach(button=>button.addEventListener('click',()=>{
      ...
      trigger=button;savedScroll={left:scrollX,top:scrollY,behavior:"instant"};dialog.classList.add("dossier-dialog");dialog.innerHTML=`<article class="dossier" id="tool-${esc(t.id)}" style="--accent:${esc(p.color)}"><button type="button" class="skill-dialog-close dossier-close" aria-label="Закрыть карточку">×</button>${dossierHead(t,p,"skill-dialog-title")}${dossierBody(t)}</article>`;
