(() => {
  ...
  async function runSearch(q){
    ...
    if(!forceAsk.has(q.trim())&&isBroad(q)){
      box.innerHTML=mapPage(q);
  ...
  function syncLiveBadge(){
    ...
    badge.innerHTML=liveOk
      ...
      : `<b>снимок</b><span>живой службы нет, ищу по локальной выгрузке словами — это не то, что делает агент. Поднять: <code>python3 scripts/mcp-proxy.py</code></span>`;
  }
