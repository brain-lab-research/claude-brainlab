  ...
  function yonote() {
    return `<div class="sp-page sp-team" data-surface-page="yonote">
      ...
      <section class="sp-section">
        ...
        <div class="sp-team-actors"><article><span class="sp-label">Человек</span><h3>Ставит и проверяет</h3><p>Определяет результат, назначает исполнителя, обсуждает прогресс и принимает выполненную работу.</p></article><article><span class="sp-label">Агент с доступом</span><h3>Готовит и обновляет</h3><p>Находит привязанную доску, проверяет будущую запись, сохраняет задачу и перечитывает результат. Повторный вызов использует тот же ключ задачи.</p></article><article><span class="sp-label">Hermes</span><h3>Получает поручение</h3><p>Его личная очередь остаётся в Hermes, обзор — в Obsidian. Работа с общей задачей требует отдельной привязки и явно порученного ID.</p></article></div>
        <p class="sp-under-map">Закрытая задача означает выполненное поручение. Оценка научного утверждения хранится отдельно в MCP вместе с измерениями или доказательством.</p>
      </section>
