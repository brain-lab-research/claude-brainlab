...
Одна запись — одна мысль, около 350 знаков. «Доказана сходимость и на GSM8K +2 пункта» — это два утверждения. `quote` — дословная фраза из сохранённого текста: служба сверяет её и несовпадение отклоняет, поэтому копируй, а не пересказывай. В `locator` пиши, откуда взято: «Математика и формулы, Theorem 2.1», «Графики и таблицы, Table 3».

Служба отказывает обрывкам, и отказ надо чинить, а не пропускать статью. Проверь сам перед записью: не короче 80 знаков и заканчивается точкой, а не двоеточием; это фраза, а не строка таблицы (числа через вертикальную черту — данные, скажи, что из них следует); не одна формула (вокруг неё не меньше 40 знаков текста); не начинается с номера пункта; у `kind: "empirical"` названа величина в любой форме: число, степень dε⁻², скорость O(1/√T), слово «вдвое» или «линейно по размерности»; не годится только пустое вроде «лучше бейзлайна». Номер рисунка величиной не считается.

Если в разборе чисел нет, а статья экспериментальная — иди на arXiv, вырежи формулировки теорем с предпосылками и таблицы с числами и положи их отдельным разделом через `add_paper_section` с именем «Оригинальный abstract (arXiv)», «Оригинальные формулировки (arXiv)» или «Оригинальные таблицы (arXiv)», и цитируй уже из него. Существующие разделы разбора не трогай: `add_paper_section` дописывает, а `upsert_paper` со списком разделов их бы стёр.
...
ОТКАЗ ИНСТРУМЕНТА — НЕ ПОВОД ПОВТОРЯТЬ ЕГО ПОДРЯД. Если вызов вернул «server is unreachable» или «Do NOT retry this tool yet», повторять его бессмысленно: служба лежит, и десятый вызов не пройдёт так же, как первый. 05-09-2026 прогон восемь раз подряд звал `record_paper_claim` в упавшую службу lab-knowledge и упёрся в предохранитель, потеряв на этом весь свой ход.
Правильный порядок: доделай всё, что не зависит от упавшей службы — заметку, карточку, рисунки, — закрой разбор, и ОТДЕЛЬНОЙ СТРОКОЙ в отчёте скажи, что именно не записалось и почему. Пропущенное подберёт следующий прогон, а брошенный на середине разбор не подберёт никто.
То же касается охранника: ответ «BLOCKED» означает, что этот путь закрыт совсем. Ищи другой способ, а не повторяй ту же команду.
