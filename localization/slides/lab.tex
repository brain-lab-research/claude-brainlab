...
\usetheme{metropolis}
\usepackage[T1]{fontenc}   % [T1] если дека только на английском
...
\begin{plumframe}
  \vfill
  \begin{center}
    {\color{amber}\fontsize{40}{46}\selectfont\bfseries Title}\\[0.45cm]
    {\color{amber!50}\rule{0.40\textwidth}{0.7pt}}\\[0.45cm]
    {\color{cream}\large A subtitle in one or two lines}
  \end{center}
  \vfill
  {\color{amber!85}\small Month Year}\hfill{\color{amber!85}\small BRAIn Lab}
  \vspace{0.3cm}
\end{plumframe}

\topicframe{01}{Section title}

% ===== Разбор работы: фигура из статьи + «что предложили» + «что не так» ======
\begin{frame}{A paper: one message in the title}
\begin{columns}[T]
\column{0.45\textwidth}
\figcard{figure.pdf}
\fcap{Illustrative comparison. State the figure's main message.}
\column{0.52\textwidth}
\begin{tcolorbox}[mathbox]
\good{The contribution.} Place the equation inside the box:
\[ z_{i+1}=f_\theta(z_i,x) \]
\end{tcolorbox}
\vspace{0.2cm}
\begin{tcolorbox}[probbox]
\bad{The limitation.} State the limitation plainly in your own words.
\end{tcolorbox}
\end{columns}
\end{frame}
